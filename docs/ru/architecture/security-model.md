...
`ENCRYPTION_SALT`. Новый шифротекст использует envelope `enc:v1:`; ошибка
расшифровки обрабатывается с блокировкой доступа, поэтому шифротекст с префиксом
или похожий на устаревший никогда не используется как пароль после
криптографической ошибки.
