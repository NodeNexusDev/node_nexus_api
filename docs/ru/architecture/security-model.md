...
`SECRET_KEY` и `ENCRYPTION_SALT`. Production startup требует минимум 32 символа
для secret и 16 для salt. Более слабые тестовые значения разрешены только при
явном `ENVIRONMENT=development` или `ENVIRONMENT=test`. Новый шифротекст использует
...
Непотоковый вывод SSH читается через ограниченные буферы до передачи в
application services. Инфраструктурные исключения возвращаются как стабильные
публичные сообщения; необработанный remote stderr не попадает в HTTP detail.
