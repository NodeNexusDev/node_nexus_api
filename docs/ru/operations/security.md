...
Учётные данные нод шифруются при сохранении с помощью AES-256-GCM на
основе `SECRET_KEY` + `ENCRYPTION_SALT`:

- `password` — SSH-пароль
- `ssh_key` — приватный SSH-ключ
- `passphrase` — passphrase для зашифрованных приватных ключей

Эти поля доступны только для записи: они никогда не возвращаются в
ответах API и исключаются из экспорта конфигурации.
