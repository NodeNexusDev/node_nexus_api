...
Расписания существуют только в памяти одного процесса API. Они теряются после
рестарта; persistent job store и координация нескольких реплик не реализованы.
...
Импортировать конфигурацию. Дубликаты по имени — включая повторы внутри одного
payload — пропускаются. Request выполняется в одной request-транзакции:
неожиданная DB-ошибка откатывает созданные элементы. Уникальность `nodes.name`
дополнительно защищена unique index.

Создание ноды с уже занятым именем возвращает `409 Conflict`.
